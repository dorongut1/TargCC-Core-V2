...
/// <summary>
/// Handles the GetRolesQuery request with server-side filtering, sorting, and pagination.
/// </summary>
public class GetRolesHandler : IRequestHandler<GetRolesQuery, Result<PagedResult<RoleDto>>>
{
    ...
    public async Task<Result<PagedResult<RoleDto>>> Handle(GetRolesQuery request, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("Getting c_Roles - Page: {Page}, Size: {Size}, SortBy: {SortBy}", request.Page, request.PageSize, request.SortBy);

        try
        {
            var query = _context.c_Roles.AsQueryable();
            ...
            _logger.LogDebug("Successfully retrieved {Count} of {Total} c_Roles", dtos.Count, totalCount);

            return Result<PagedResult<RoleDto>>.Success(result);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error retrieving c_Roles");
            throw;
        }
    }

    private static IQueryable<Role> ApplyFilters(IQueryable<Role> query, RoleFilters? filters)
    {
        if (filters == null)
        {
            return query;
