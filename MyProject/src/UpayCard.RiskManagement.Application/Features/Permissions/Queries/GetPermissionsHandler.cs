...
namespace UpayCard.RiskManagement.Application.Features.c_Permissions.Queries;
...
public class GetPermissionsHandler : IRequestHandler<GetPermissionsQuery, Result<PagedResult<PermissionDto>>>
{
    ...
    public async Task<Result<PagedResult<PermissionDto>>> Handle(GetPermissionsQuery request, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("Getting c_Permissions - Page: {Page}, Size: {Size}, SortBy: {SortBy}", request.Page, request.PageSize, request.SortBy);

        try
        {
            var query = _context.c_Permissions.AsQueryable();
            ...
            _logger.LogDebug("Successfully retrieved {Count} of {Total} c_Permissions", dtos.Count, totalCount);

            return Result<PagedResult<PermissionDto>>.Success(result);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error retrieving c_Permissions");
            throw;
        }
    }
