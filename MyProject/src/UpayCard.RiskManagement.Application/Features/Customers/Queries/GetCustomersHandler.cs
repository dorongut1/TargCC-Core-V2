...
/// <summary>
/// Handles the GetCustomersQuery request with server-side filtering, sorting, and pagination.
/// </summary>
public class GetCustomersHandler : IRequestHandler<GetCustomersQuery, Result<PagedResult<CustomerDto>>>
{
    ...
    public async Task<Result<PagedResult<CustomerDto>>> Handle(GetCustomersQuery request, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("Getting Customers - Page: {Page}, Size: {Size}, SortBy: {SortBy}", request.Page, request.PageSize, request.SortBy);
        ...
        try
        {
            ...
            _logger.LogDebug("Successfully retrieved {Count} of {Total} Customers", dtos.Count, totalCount);
    ...
    private static IQueryable<Customer> ApplyFilters(IQueryable<Customer> query, CustomerFilters? filters)
    {
        if (filters == null)
        {
            return query;
