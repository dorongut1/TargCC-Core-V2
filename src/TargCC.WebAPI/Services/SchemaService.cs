...
public class SchemaService : ISchemaService
{
    ...
    public async Task<List<string>> GetSchemasAsync(string connectionString)
    {
        ...
        var schemas = await connection.QueryAsync<string>(sql, commandTimeout: 300);
    ...
    private async Task<List<TableDto>> GetTablesAsync(IDbConnection connection, string schemaName)
    {
        ...
        var tables = (await connection.QueryAsync<TableDto>(sql, new { SchemaName = schemaName }, commandTimeout: 300)).ToList();
    ...
    private async Task<List<ColumnDto>> GetColumnsAsync(IDbConnection connection, string schemaName, string tableName)
    {
        ...
        var columns = await connection.QueryAsync<ColumnDto>(sql, new { SchemaName = schemaName, TableName = tableName }, commandTimeout: 300);
    ...
    private async Task<List<RelationshipDto>> GetRelationshipsAsync(IDbConnection connection, string schemaName)
    {
        ...
        var relationships = await connection.QueryAsync<RelationshipDto>(sql, new { SchemaName = schemaName }, commandTimeout: 300);
    ...
    public async Task<TablePreviewDto> GetTablePreviewAsync(string connectionString, string schemaName, string tableName, int rowCount = 10)
    {
        ...
        var columns = (await connection.QueryAsync<string>(columnsSql, new { SchemaName = schemaName, TableName = tableName }, commandTimeout: 300)).ToList();
        ...
        var data = (await connection.QueryAsync(dataSql, commandTimeout: 300)).Select(row =>
        {
        ...
        var totalCount = await connection.ExecuteScalarAsync<int>(countSql, commandTimeout: 300);
