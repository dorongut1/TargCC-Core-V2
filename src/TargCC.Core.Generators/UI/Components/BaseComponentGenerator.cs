...
namespace TargCC.Core.Generators.UI.Components
{
    ...
    public abstract class BaseComponentGenerator : BaseUIGenerator, IComponentGenerator
    {
        ...
        protected string GenerateComponentHeader(string tableName)
        {
        ...
        protected static List<string> GetValidationRules(Column column)
        {
            ...
            {
                rules.Add(CultureInfo.InvariantCulture, $"maxLength: {{ value: {column.MaxLength.Value}, message: 'Maximum length is {column.MaxLength.Value}' }}");
            }

            // Pattern validation
            var pattern = GetValidationPattern(column, prefix);
        ...
        private static string GetValidationPattern(Column column, string prefix)
        {
            var sqlType = column.DataType.ToUpperInvariant();
