...
namespace TargCC.Core.Generators.UI.Components
{
    ...
    public class ReactComponentOrchestratorGenerator : BaseUIGenerator
    {
            ...
            ComponentGeneratorConfig config)
        {
            ...
            // Generate routes
            var routes = GenerateRoutes(table, config);
        ...
        private static string GenerateRoutes(Table table, ComponentGeneratorConfig config)
        {
            var sb = new StringBuilder();
            var className = GetClassName(table.Name);
        ...
        private static string GenerateRootRoutingConfig(DatabaseSchema schema)
        {
            ...
            // Import all routes
            foreach (var table in schema.Tables.Take(10)) // Limit to first 10 for example
            {
                var className = GetClassName(table.Name);
        ...
        private async Task<string> GenerateAllComponentsAsync(Table table, DatabaseSchema schema, ComponentGeneratorConfig config)
        {
            var components = await GenerateAllComponentsAsync(table, schema, config).ConfigureAwait(false);

            var sb = new StringBuilder();
            sb.AppendLine(CultureInfo.InvariantCulture, $"Generated {components.Count} files for {table.Name}:");
            foreach (var file in components.Keys)
            {
