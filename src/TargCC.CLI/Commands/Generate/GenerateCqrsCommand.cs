...
/// <summary>
/// Command to generate CQRS commands, queries, and handlers for a database table.
/// </summary>
public class GenerateCqrsCommand : Command
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="GenerateCqrsCommand"/> class.
    /// </summary>
    /// <param name="logger">Logger instance.</param>
    /// <param name="configService">Configuration service.</param>
    /// <param name="output">Output service.</param>
    /// <param name="generationService">Generation service.</param>
    public GenerateCqrsCommand(
        ILogger logger,
        IConfigurationService configService,
        IOutputService output,
        IGenerationService generationService)
        : base("cqrs", "Generate CQRS commands, queries, and handlers for a database table")
    {
        ...
        string? outputDirectory)
    {
        try
        {
            _output.Heading($"Generate CQRS: {tableName}");
            _output.BlankLine();
            ...
            await _output.SpinnerAsync(
                "Generating CQRS components...",
                async () =>
                {
                    result = await _generationService.GenerateCqrsAsync(
                        connectionString,
                        tableName,
                        outputDirectory);
                });
        ...
        {
            _logger.LogError(ex, "Error in generate cqrs command");
