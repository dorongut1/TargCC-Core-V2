...
namespace UpayCard.RiskManagement.Application.Features.Invoices.Queries;

/// <summary>
/// Handles the GetInvoicesQuery request with server-side filtering, sorting, and pagination.
/// </summary>
public class GetInvoicesHandler : IRequestHandler<GetInvoicesQuery, Result<PagedResult<InvoiceDto>>>
{
    ...
    public async Task<Result<PagedResult<InvoiceDto>>> Handle(GetInvoicesQuery request, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("Getting Invoices - Page: {Page}, Size: {Size}, SortBy: {SortBy}", request.Page, request.PageSize, request.SortBy);
        ...
        try
        {
            ...
            var dtos = _mapper.Map<List<InvoiceDto>>(entities);
            ...
            _logger.LogDebug("Successfully retrieved {Count} of {Total} Invoices", dtos.Count, totalCount);
    ...
    private static IQueryable<Invoice> ApplyFilters(IQueryable<Invoice> query, InvoiceFilters? filters)
    {
        if (filters == null)
        {
            return query;
    ...
    private static IQueryable<Invoice> ApplySorting(IQueryable<Invoice> query, string? sortBy, string? sortDirection)
    {
